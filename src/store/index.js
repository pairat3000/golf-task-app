...
function applyDoneDate(task, newStatus) {
  const updated = { ...task, status: newStatus }
  if (newStatus === 'Done' && task.status !== 'Done' && !task.dateDone) {
...
export const useStore = create(
  persist(
    (set, get) => ({
      ...
      aiPlan: null,
      aiLoading: false,
      aiChatHistory: [],   // persisted across sessions
      ...
      addProject: (project) =>
        set(s => ({
          projects: [...s.projects, { ...project, id: makeId('p'), createdAt: new Date().toISOString() }]
        })),
      ...
      moveTask: (taskId, newStatus) =>
        set(s => ({
          tasks: s.tasks.map(t => t.id === taskId ? applyDoneDate(t, newStatus) : t)
        })),

      // ── Subtask actions ────────────────────────────────────────────────────
      addSubtask: (taskId, title) => {
        const subtask = { id: makeId('st'), title, done: false, createdAt: new Date().toISOString() }
        set(s => ({
      ...
      addComment: (taskId, text, author = 'Golf') => {
        const comment = { id: makeId('c'), text, author, createdAt: new Date().toISOString() }
        set(s => ({
          tasks: s.tasks.map(t => t.id === taskId
      ...
      updateRequirement: (id, data) =>
        set(s => ({ requirements: s.requirements.map(r => r.id === id ? { ...r, ...data } : r) })),
      ...
      addChatMessage: (role, content) =>
        set(s => ({
          aiChatHistory: [
            ...s.aiChatHistory,
            { role, content, id: makeId('msg'), createdAt: new Date().toISOString() }
          ]
        })),
      ...
      // ── Backup/Restore ─────────────────────────────────────────────────────
      exportData: () => {
        const { projects, tasks, requirements } = get()
        const data = { projects, tasks, requirements, exportedAt: new Date().toISOString(), version: '1.1' }
